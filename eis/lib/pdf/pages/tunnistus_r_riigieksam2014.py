...
<i><u>English:</u> Preliminary English Test (PET), First Certificate in English (FCE), Certificate in Advanced English (CAE), Certificate of Proficiency in English (CPE), International English Language Testing System (IELTS) (min 4.0 p), Test of English as a Foreign Language (TOEFL) iBT (min 57 p); <u>French:</u> DALF C1; <u>German:</u> Goethe-Zertifikat C1, Goethe-Zertifikat C2, TestDaF (TDN3), TestDaF (TDN4), TestDaF (TDN5); Österreichisches Sprachdiplom Zertifikat B1, Österreichisches Sprachdiplom B2 Mittelstufe Deutsch, Österreichisches Sprachdiplom C1 Oberstufe Deutsch; Deutsches Sprachdiplom I B1.</i>"""
...
def tulemus_rida(sooritaja, test, ingl_tase_kood):
    def get_kpv(sooritaja_id):
        "Leiame testi kuupäeva. Kui toimub mitmel päeval, siis kasutame kirjaliku osa kuupäeva."
        algus = None
        q_kpv = model.Session.query(model.Sooritus.algus, model.Testiosa.vastvorm_kood).\
    ...
    s_kpv = helpers.str_from_date(kuupaev)
    testikursus = test.get_testikursus(sooritaja.kursus_kood)
    aine_nimi = testikursus and testikursus.tunnaine_nimi or test.aine_nimi
    if 'iÕK' in aine_nimi or 'IÕK' in aine_nimi:
        iOK = True
        aine_nimi += '*'

    if test.aine_kood == const.AINE_EN:
        # inglise keele korral on kogutulemuse asemel ainult tase
        ingl_tase_kood = sooritaja.keeletase_kood
        tase = sooritaja.keeletase_nimi
        if tase:
            tulemus = 'Saavutatud keeleoskustase %s' % tase
        else:
            tulemus = 'Keeleoskustase saavutamata'
    else:
        if sooritaja.staatus != const.S_STAATUS_TEHTUD:
            pallid = '0 punkti'
        elif test.aine_kood == const.AINE_ET2:
            # EH-244, ES-2834
            pallid = helpers.fstr(sooritaja.tulemus_protsent, 2) + ' %'
        else:
            pallid = helpers.fstr(sooritaja.pallid, 2) + ' punkti'
        max_pallid = helpers.fstr(sooritaja.test.max_pallid, 2) + ' punkti'

        # testi kogutulemus
        tulemus = '%s / %s' % (pallid, max_pallid)

    return s_kpv, aine_nimi, tulemus, iOK, ingl_tase_kood
