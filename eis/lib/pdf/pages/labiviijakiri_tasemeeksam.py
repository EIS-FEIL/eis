...
from .labiviijaakt_tasemeeksam import labiviija_tbl
...
def generate(story, items, grupid_id, testiliik, taiendavinfo):
    ...
    story.append(Paragraph(_('Lugupeetud {s}, edastame Teile andmed vastavalt teenuse osutamise lepingule Teie poolt eesti keele tasemeeksamitel teostatud tööde ja väljamaksmisele kuuluvate töötasude kohta.').format(s='%s %s' % (k_eesnimi, k_perenimi)), M))
    story.append(Spacer(1,3*mm))
    ...
    buf = _('Kokku arvestatud {m} eurot ({s1} eurot ja {s2} senti).').format(
        m=h.mstr(tasu), s1=utils.arvsonades(eurod), s2=utils.arvsonades(sendid))
