...
def generate(story, items, grupid_id, testiliik, taiendavinfo):
    ...
    story.append(Paragraph(_('Lugupeetud {s}, edastame Teile andmed vastavalt teenuse osutamise lepingule Teie poolt Eesti Vabariigi põhiseaduse ja kodakondsuse seaduse tundmise eksamitel teostatud tööde ja väljamaksmisele kuuluvate töötasude kohta.').format(s='%s %s' % (k_eesnimi, k_perenimi)), M))    
    story.append(Spacer(1,3*mm))
