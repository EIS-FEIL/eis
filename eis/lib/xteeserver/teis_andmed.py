"""
Tasemeeksami soorituste ja registreeringute andmete väljastamine riigiportaalis ametnikule.
...
def teis_andmed_x(ik):
    ...
    if not k:
        ...
        # kuvame tasemeeksamid (ilmselt aines "riigikeel")
        # ja riigieksamid ja põhikooli lõpueksamid aines "eesti keel teise keelena"
        # kuvame ainult keeletasemega eksamid (vanadel riigieksamitel keeletaset pole
        # ja neid pole vaja kuvada, kuna nende asemel on TSEISist imporditud fiktiivsed eksamid)
        q = q.filter(model.Test.testiliik_kood.in_((const.TESTILIIK_TASE,
                                                    const.TESTILIIK_RIIGIEKSAM,
                                                    const.TESTILIIK_POHIKOOL))).\
            filter(model.Test.aine_kood.in_((const.AINE_ET2, const.AINE_RK))).\
            filter(model.Test.testitasemed.any()).\
        ...
        for r in q.all():
            test, algus, staatus, h_staatus, pallid, tulemus_protsent, kinnitatud, avaldet = r

            tulemus = ''
            if staatus == const.S_STAATUS_TEHTUD:
                if kinnitatud and avaldet:
                    staatus = 'Tulemus on teada'
                    tulemus = h.fstr(tulemus_protsent) or ''
                else:
                    staatus = 'Tulemus ei ole teada'
            else:
                staatus = const.S_STAATUS.get(staatus)
            
            li.append(E.item(E.tase(test.keeletase_nimi or ''),
                             E.aeg(h.str_from_date(algus) or ''),
                             E.staatus(staatus),
                             E.tulemus(tulemus)))
        res.append(li)
        ...
        q = q.filter(model.Tunnistus.kasutaja_id==k.id).\
            filter(model.Tunnistus.testiliik_kood==const.TESTILIIK_TASE).\
        ...
        # tasemeeksamile registreerimise staatused
        li = E.reg_staatus_jada()
        q = model.Session.query(model.Test, model.Sooritaja)
        q = q.filter(model.Test.testiliik_kood==const.TESTILIIK_TASE).\
            join(model.Test.sooritajad).\
            filter(model.Sooritaja.kasutaja_id==k.id).\
        ...
        for r in q.all():
            test, sooritaja = r
            koht = '%s %s / %s' % (test.keeletase_nimi or '',
