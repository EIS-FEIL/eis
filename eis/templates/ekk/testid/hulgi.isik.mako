...
${h.form_search(url=h.url('testid_hulgi_isikud', testid_id=c.testid_id))}
<table class="table"  width="100%">
  ...
  <tr>
    ...
    <td>
      ${h.button(_("Otsi"), onclick="var url='%s?'+$(this.form).serialize();dialog_load(url);" % h.url('testid_hulgi_isikud', testid_id=c.testid_id))}
    </td>
  </tr>
</table>
${h.end_form()}
...
% if c.items:
${h.form(h.url('testid_hulga', id=c.testid_id), method='put')}
${h.hidden('sub', 'isik')}
  ...
  <tr>
    ...
    <td>
      <div style="margin:7px 0">
        ${h.select('kasutajagrupp_id', '', c.opt.testigrupp)}
      </div>
      <div>
