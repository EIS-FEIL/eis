...
<table width="100%" class="table table-borderless table-striped" border="0" >
  <thead>
    <tr>
      ...
      ${h.th(_('Kinnitatud või üle vaadatud'))}
      ${h.th(_('Ül komplekt'))}
    </tr>
  </thead>
  <tbody>
    ...
    <% 
       ...
       url_edit = h.url('erivajadus', id=tos.id)
    %>
    <tr>
      ...
      <td>
        ${h.link_to(h.literal(tos.get_str_erivajadused('<br/>') or _('Märkimata')), url_edit)}
      </td>
      <td>${h.sbool(tos.on_erivajadused_kinnitatud or tos.on_erivajadused_vaadatud)}</td>
