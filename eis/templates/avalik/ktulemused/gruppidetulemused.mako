...
<%def name="page_title()">
${c.test.nimi or ''} | ${_("Gruppide tulemused")}
</%def>      
<%def name="breadcrumbs()">
${h.crumb(_("Tsentraalsed testid"))}
${h.crumb(_('Tulemused'), h.url('ktulemused'))}
${h.crumb(c.test.nimi, h.url('ktulemused_gruppidetulemused', test_id=c.test.id, testimiskord_id=c.testimiskord.id, kursus=c.kursus or ''))}
</%def>
