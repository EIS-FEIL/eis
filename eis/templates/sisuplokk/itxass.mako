...
${choiceutils.edit_hotspots(c.block, kysimus=prkkysimus, invisible=True, maxvastus=False, yle=False)}
% endif
...
<%
  ...
  prkkysimus = c.block.get_baaskysimus(2)
  min_choice_info = []
...
%>
  ...
  ${graphutils.js_show_hotspots(c.block, kysimus=prkkysimus)}
  sketchpad.make_txelems($('#dragtbl_${c.block_prefix}'), false, ${kysimus.min_vastus or 0}, ${kysimus.max_vastus or 0}, ${mo.masonry_layout and 'true' or 'false'});
  sketchpad.make_txelems_nodes(${mo.asend});
  % if c.is_edit:
  sketchpad.make_txelems_associable();
  % endif
% if min_choice_info:
...
## liigutame pildid sinna, kuhu kasutaja need pani
  % for kood, kv in responses.items():
    ...
    %>
    % if kysimus and kysimus.sisuplokk_id == c.block.id:
       % for n, ks in enumerate(kv.kvsisud):
         % if ks.kood1:
  var line = sketchpad.associate(sketchpad.find_shape_node('${h.chk_w(kood)}','txassociable'),
                                 sketchpad.find_shape_node('${h.chk_w(ks.kood1)}', 'associable'),
                                 true);
